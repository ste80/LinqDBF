...
namespace LinqDBF
{
    public class MemoValue
    {
        ...
        {
            _lockName = $"LinqDBF.Memo.new.{Guid.NewGuid()}";
            Value = aValue;
        }


        //internal MemoValue(long block, DBFBase aBase, string fileLoc, DBFReader.LazyStream fileStream)
        internal MemoValue(long block, DBFBase aBase, Lazy<Stream> fileStream)
        {
            _block = block;
            _base = aBase;
            //_fileLoc = fileLoc;
            _FileStream = fileStream.Value;
            //if (String.IsNullOrEmpty(fileLoc))
            //{
            _lockName = fileStream.Value;
            //}
            //else
            //{
            //    _lockName = $"LinqDBF.Memo.read.{_fileLoc}";
            //}
        }
